...
edata<-NULL
data2<-read.table('10r10f_bugzilla.csv',header=TRUE,sep=",")
edata<-rbind(edata, data2)
data2<-read.table('10r10f_columba.csv',header=TRUE,sep=",")
edata<-rbind(edata, data2)
data2<-read.table('10r10f_jdt.csv',header=TRUE,sep=",")
edata<-rbind(edata, data2)
data2<-read.table('10r10f_mozilla.csv',header=TRUE,sep=",")
edata<-rbind(edata, data2)
data2<-read.table('10r10f_platform.csv',header=TRUE,sep=",")
edata<-rbind(edata, data2)
data2<-read.table('10r10f_postgres.csv',header=TRUE,sep=",")
edata<-rbind(edata, data2)

edata<-edata[edata$Method!="Random",]
summary(edata)
write.csv(edata, file = "10r10f_rawdata.csv")
...
ggplot(edata)+
  geom_raster(aes(x=round(AUC,2), y=round(ACC,2), fill=MethodType))+
  stat_smooth(aes(x=AUC, y=ACC, color=MethodType), method="glm", se=F)+labs(x="AUC",y="ACC")
ggplot(edata)+
  geom_raster(aes(x=round(AUC,2), y=round(Popt,2), fill=MethodType))+
  stat_smooth(aes(x=AUC, y=Popt, color=MethodType), method="glm", se=F)+labs(x="AUC",y="Popt")
